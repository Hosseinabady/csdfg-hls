...
int main(int argc, char *argv[])
{
	...
	if ( (y_hw=(DATA_TYPE *)malloc(n*sizeof(DATA_TYPE))) == NULL )
		perror("memory allocation for a");
	if ( (A_hw=(DATA_TYPE *)malloc(n*m*sizeof(DATA_TYPE))) == NULL )
		perror("memory allocation for b");
	if ( (x_hw=(DATA_TYPE *)malloc(n*sizeof(DATA_TYPE))) == NULL )
		perror("memory allocation for c");
	if ( (d_hw=(DATA_TYPE *)malloc(n*sizeof(DATA_TYPE))) == NULL )
		perror("memory allocation for a");
	...
	hv(
		(DATA_TYPE_WB*)y_hw,
		(DATA_TYPE_WB*)(A_hw+0*n*m/4),
		(DATA_TYPE_WB*)(A_hw+1*n*m/4),
		(DATA_TYPE_WB*)(A_hw+2*n*m/4),
		(DATA_TYPE_WB*)(A_hw+3*n*m/4),
		(DATA_TYPE_WB*)x_hw,
		(DATA_TYPE_WB*)d_hw,
		m,
		n
		...
		DATA_TYPE *  x, DATA_TYPE *  d)
{
   ...
   for (i=0; i<m; i++)
   {
	   y_local[i] = 0.0;
      for (j=0; j<n; j+=5)
    	  y_local[i] += A[i*m+j+0]*x[j+0] + A[i*m+j+1]*x[j+1] + A[i*m+j+2]*x[j+2] + A[i*m+j+3]*x[j+3] + A[i*m+j+4]*x[j+4];
