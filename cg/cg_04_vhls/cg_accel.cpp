...
void init(DATA_TYPE_WB *A_0,  DATA_TYPE_WB *A_1,  DATA_TYPE_WB *A_2,  DATA_TYPE_WB *A_3,  DATA_TYPE *b, DATA_TYPE *x, DATA_TYPE *r, DATA_TYPE *p, u32 n) {


#pragma HLS DATAFLOW
	hls::stream<DATA_TYPE_WB> a_fifo[4];
#pragma HLS ARRAY_PARTITION variable=a_fifo complete dim=1
	hls::stream<DATA_TYPE> ax_fifo[4];
#pragma HLS ARRAY_PARTITION variable=ax_fifo complete dim=1
...
#pragma HLS PIPELINE
			a_fifo[0] << A_0[(i*n+j)/NUMBER_OF_DATATYPE_IN_WB];
			a_fifo[1] << A_1[(i*n+j)/NUMBER_OF_DATATYPE_IN_WB];
			a_fifo[2] << A_2[(i*n+j)/NUMBER_OF_DATATYPE_IN_WB];
			a_fifo[3] << A_3[(i*n+j)/NUMBER_OF_DATATYPE_IN_WB];
		}
	}
...
#pragma HLS ARRAY_PARTITION variable=tmp complete dim=1
			for (int t = 0; t < 4; t++) {
				...
				a_local_1.data = a_fifo[t].read();

				WIDE_DATA_TYPE_TO_DATA_TYPE a_local_2;
				a_local_2.data = a_fifo[t].read();

				WIDE_DATA_TYPE_TO_DATA_TYPE a_local_3;
				a_local_3.data = a_fifo[t].read();

				WIDE_DATA_TYPE_TO_DATA_TYPE a_local_4;
				a_local_4.data = a_fifo[t].read();

				WIDE_DATA_TYPE_TO_DATA_TYPE a_local_5;
				a_local_5.data = a_fifo[t].read();



				tmp[t] = a_local_1.float_data.a*x[j+0] + a_local_1.float_data.b*x[j+1] +
	...
	for(u32 j = 0; j < n/4; j++) {
#pragma HLS PIPELINE
		DATA_TYPE ax_local[4];
#pragma HLS ARRAY_PARTITION variable=ax_local complete dim=1
		for (int t = 0; t < 4; t++) {
			ax_local[t] = ax_fifo[t].read();
			DATA_TYPE tmp = b[j+t*n/4]-ax_local[t];
			r[j+t*n/4] = tmp;
			p[j+t*n/4] = tmp;
		}
	}
...
void iteration(DATA_TYPE_WB *A_0,  DATA_TYPE_WB *A_1,  DATA_TYPE_WB *A_2,  DATA_TYPE_WB *A_3,  DATA_TYPE *b, DATA_TYPE *x_in, DATA_TYPE *x_out, DATA_TYPE *r_in, DATA_TYPE *r_out, DATA_TYPE *p_in,  DATA_TYPE *p_out, u32 n) {
#pragma HLS DATAFLOW
...
#pragma HLS STREAM variable=r_fifo_4 depth=FIFO_WIDTH dim=1
	...
	hls::stream<DATA_TYPE> q_fifo_1[4];
#pragma HLS STREAM variable=q_fifo_1 depth=FIFO_WIDTH dim=1
#pragma HLS ARRAY_PARTITION variable=q_fifo_1 complete dim=1
...
#pragma HLS STREAM variable=p_fifo_1 depth=FIFO_WIDTH dim=1
...
#pragma HLS PIPELINE
		...
		if (j < 1*n/4) {
			q_tmp_1 = q_fifo_1[0].read();
			q_tmp_2 = q_fifo_1[0].read();

		} else if (j < 2*n/4) {
			q_tmp_1 = q_fifo_1[1].read();
			q_tmp_2 = q_fifo_1[1].read();

		} else if (j < 3*n/4) {
			q_tmp_1 = q_fifo_1[2].read();
			q_tmp_2 = q_fifo_1[2].read();

		} else  {
			q_tmp_1 = q_fifo_1[3].read();
			q_tmp_2 = q_fifo_1[3].read();
...
u32 cg_accel(
		DATA_TYPE_WB              A_0[N*N/(NUMBER_OF_DATATYPE_IN_WB*4)],
		DATA_TYPE_WB              A_1[N*N/(NUMBER_OF_DATATYPE_IN_WB*4)],
		DATA_TYPE_WB              A_2[N*N/(NUMBER_OF_DATATYPE_IN_WB*4)],
		DATA_TYPE_WB              A_3[N*N/(NUMBER_OF_DATATYPE_IN_WB*4)],
...
#pragma HLS INTERFACE m_axi depth=1024 port=A_0 offset=slave bundle=gemm0
#pragma HLS INTERFACE m_axi depth=1024 port=A_1 offset=slave bundle=gemm1
#pragma HLS INTERFACE m_axi depth=1024 port=A_2 offset=slave bundle=gemm2
#pragma HLS INTERFACE m_axi depth=1024 port=A_3 offset=slave bundle=gemm3
...
#pragma HLS INTERFACE s_axilite port=A_0 bundle=bus_controll
#pragma HLS INTERFACE s_axilite port=A_1 bundle=bus_controll
#pragma HLS INTERFACE s_axilite port=A_2 bundle=bus_controll
#pragma HLS INTERFACE s_axilite port=A_3 bundle=bus_controll
...
#pragma HLS PIPELINE
	...
	init(A_0, A_1, A_2, A_3, b_local, x_local_1, r_1, p_1, n);
	for (u32 iter = 0; iter < MAX_ITERATIONS; iter++) {
		iteration(A_0, A_1, A_2, A_3, b_local, x_local_1, x_local_2, r_1, r_2, p_1, p_2, n);
		iteration(A_0, A_1, A_2, A_3, b_local, x_local_2, x_local_1, r_2, r_1, p_1, p_2, n);
	}
