...
void cg_golden(  DATA_TYPE       A[N*N]
	...
	for (int i = 0; i <n; i++)  {
		r[i] = 0;
		for (int j = 0; j <n; j+=5) {
			r[i] += A[i*n+j+0]*x[j+0] + A[i*n+j+1]*x[j+1] + A[i*n+j+2]*x[j+2] + A[i*n+j+3]*x[j+3] + A[i*n+j+4]*x[j+4];
		}
		r[i] = b[i] - r[i];
	}
	...
	for (int iter = 0; iter < 2*MAX_ITERATIONS; iter++) {
		...
		for (int i = 0; i <n; i++)  {
			q[i] = 0;
			for (int j = 0; j <n; j+=5) {
				q[i] += A[i*n+j+0]*p[j+0] + A[i*n+j+1]*p[j+1] + A[i*n+j+2]*p[j+2] + A[i*n+j+3]*p[j+3] + A[i*n+j+4]*p[j+4];
			}
		}
